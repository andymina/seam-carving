...
Seam CarvableImage::FindOptimalSeam(const Dir &dir) {
    return (dir == VERT) ? this->__FindOptimalVerticalSeam(this->res_img) : this->__FindOptimalHorizontalSeam(this->res_img);
}
...
void CarvableImage::InsertSeam(const Dir &dir) {
    ...
    this->res_img = (dir == VERT) ? this->__InsertVerticalSeam(seam, this->res_img) : this->__InsertHorizontalSeam(seam, this->res_img);
